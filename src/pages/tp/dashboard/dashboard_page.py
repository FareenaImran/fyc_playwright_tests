...
class TPDashboardPage(SideMenu):
    PROFILE_STATUS = "(//p[contains(text(),'Profile Status')]/following-sibling::div/p)[1]"
    FEEDBACK_MSG="(//p[contains(text(),'profile needs attention')]/following-sibling::div//p)[1]"
    # Navigate to courses
    async def navigate_to_courses(self):
        await check_and_close_page_modal(self.page)
        await self.courses.first.click()
